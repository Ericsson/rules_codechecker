...
- **\[version_tags\]** A list of bazel versions and project hashes to test on.
    - \[bazel_version\] The major bazel version to be tested on as a string. (Default to False on bazel 6, 7 True on bazel 8 and onwards)
    - \[hash\] The tag or commit hash to be checked out for this bazel version.
    - \[bzlmod\] Boolean wether to use the bzlmod (`MODULE.bazel`) or the legacy (`WORKSPACE`) system
